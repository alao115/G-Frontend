export default ({ apollo, gql, $axios }) => class AppartmentService {
  getAll ({ projections, variables } = { projections: 'id createdBy { id user { userType emailVerified } status firstname lastname civility phone email } mainImg firstImg secondImg thirdImg fourthImg appartmentType forShortStay isFurnished location rent details rooms bathrooms bedrooms livingrooms storageroom kitchen garage keeper terrace garden ac pool householdsTotal groundLevel conditions { advancePayment energyCommission prepaidRentMonths paymentFrequency } ownerInfos { civility firstname lastname address isAlive phone email } likes favorite', variables: {} }) {
